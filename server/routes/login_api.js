...
router.post('/api/authentication', (req, res) => {
  ...
  if(req.body.userId.toUpperCase()  == "123" && req.body.password == "123"){
			console.log("User Authentication is SUCCESS !");
		    res.json({authSuccess : true,message :"User Authentication Successful" , role : "KAPY_Surveyor"});
		}else{
  ...
  //logindb.find({selector:{userId: req.body.userId}}, function(er, result) {
	...
	//	if(userInfoForm.userId.toUpperCase()  == "123" && userInfoForm.password == "123"){
	//		console.log("User Authentication is SUCCESS !");
